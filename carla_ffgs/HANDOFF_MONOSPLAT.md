# Handoff: run **MonoSplat** zero-shot on the CARLA **CSE** benchmark

You're working **inside the cloned `MonoSplat` repo**
(https://github.com/CUHK-AIM-Group/MonoSplat, CVPR'25).
Goal: evaluate MonoSplat **zero-shot** (official RealEstate10K weights) on our CARLA
**CSE (Cross-Sensor)** benchmark and score it with our `eval_cse.py`, so the
numbers are directly comparable to GS-Net and 3DGS.

> MonoSplat is a **reviewer-named** generalizable feed-forward 3DGS method whose
> geometric prior comes from a **monocular depth foundation model** (so it also
> covers the "use a depth/geometry prior" request). It is built on the
> **pixelSplat / MVSplat codebase and shares the same `.torch` data format**, so the
> converter below — and the whole integration — is the **same as the MVSplat /
> DepthSplat sibling tasks**.
...
runs/cse_scenes/<id>/  ──(1) colmap_to_pixelsplat.py──►  datasets/carla_cse/test/{*.torch, index.json, cse_meta.json}
                       ...
                       ──(2) export──►  monosplat/<id>/renders/<target_name>.png   (named exactly per test.txt)
                       ──(3) eval_cse.py──►  PSNR / SSIM / LPIPS
```

You implement: **(1)** the converter (code provided, just save & run), **(2)** a
CARLA-CSE dataset + view-sampler + a render-export script that reuses MonoSplat's
...
- Each scene is a standard COLMAP text model:
  ...
  runs/cse_scenes/<id>/
    ...
    sparse/0/points3D.*    # source (odd) sparse SfM points (optional, for near/far)
...
- **Pose convention (COLMAP, OpenCV)**: `R = qvec2rotmat(q)` is world→camera,
  ...
  pixelSplat/MVSplat/MonoSplat use** (store world→camera, invert internally) → **no
  conversion needed**.
...
`.torch` chunk format used by MVSplat / DepthSplat / MonoSplat.
...
**Reuse MonoSplat's own encoder + decoder.** MonoSplat keeps pixelSplat/MVSplat's
abstractions (`Dataset`, `ViewSampler`, `Encoder`, `Decoder`). Read these repo files
first to match the exact API:
- `src/dataset/dataset_re10k.py` — how a batch is built: `context`/`target` dicts
  ...
  [B,V,4,4]` (cam-to-world, OpenCV), `near`, `far`. **Copy its pose-normalization and
  image-resize logic verbatim.**
- `src/dataset/view_sampler/` — `ViewSampler` base + `view_sampler_evaluation.py`
  (MonoSplat ships `assets/evaluation_index_*.json` files of context/target indices).
- `src/model/` — the encoder fuses a **monocular depth foundation model** with
  multi-view features (needs the UniMatch backbone); `encoder(context) -> gaussians`;
  the decoder renders the target view. Confirm the exact `forward` signature.
...
2. **`ViewSamplerCSE`**: one item per target view. For target index `t`, context =
   ...
   the source `cam_centers` (source = `is_target==False`). Return
   `context_indices` (K nearest sources), `target_indices=[t]`.

   *Shortcut*: MonoSplat natively supports `dataset/view_sampler=evaluation` reading
   a JSON `{scene: {"context": [...], "target": [...]}}`. You can instead have the
   converter emit such an index (per target, K nearest sources) and reuse the repo's
   evaluation sampler — whichever is less code.

**Number of context views**: MonoSplat's released re10k indices use varying `nctx`
(e.g. `nctx2` … `nctx10`), so the model tolerates 2–10 context views. Start with
**K=2** (the standard 2-view interpolation regime, two flanking source cams); you
may also try **K=4–6** spatially-spread source views and report the best.
...
`outputs/monosplat/<scene_id>/renders/<names[t]>` (PNG, exact target filename from
`cse_meta.json`/`test.txt`). 60 files per scene.

Drive it with a small `src/scripts/render_cse.py` that builds the config,
instantiates `DatasetCarlaCSE` + `ViewSamplerCSE`, loads the checkpoint via the
repo's model wrapper, loops batches, renders, and writes PNGs.
...
python eval_cse.py --multi \
  outputs/monosplat/110/renders:runs/cse_scenes/110 \
  outputs/monosplat/210/renders:runs/cse_scenes/210 \
  outputs/monosplat/310/renders:runs/cse_scenes/310 \
  outputs/monosplat/410/renders:runs/cse_scenes/410 \
  outputs/monosplat/510/renders:runs/cse_scenes/510 \
  --out outputs/monosplat/cse_scores.json
```
...
- Env: follow the repo (conda, Python 3.10, **PyTorch 2.1.2 / CUDA 11.8**). Needs a CUDA GPU.
- The encoder needs the **UniMatch** pretrained backbone and the **monocular depth
  foundation model** — follow the repo's setup to download/place them (feature
  backbones, **not GT depth**; no ground-truth depth required for zero-shot inference).
- Gaussian checkpoint — the **SharePoint link in the MonoSplat README**, save to
  `checkpoints/`. The released model is trained on **RealEstate10K (indoor)**.
- Use **2 context views** for the headline number; optionally also K=4–6.
...
reviewers asked for. The fair, in-domain number will come later from a **CARLA
fine-tuned** run (training scenes 101–109 … 501–509, RGB+pose only — MonoSplat
trains with a photometric rendering loss; not part of this task yet).

## Acceptance checklist
- [ ] Converter prints `60 src / 60 tgt` for all 5 scenes.
- [ ] `renders/` has exactly 60 PNGs per scene, names == `test.txt`, shape == GT.
- [ ] UniMatch + mono-depth backbones load; checkpoint loads without key errors.
